...
const PhotosUploader = ({ photo, onChange }) => {
  ...
  const removePhoto = (ev, filename) => {
    ev.preventDefault();
    onChange([...photo.filter((photo) => photo !== filename)]);
  };

  const selectAsMainPhoto = (ev, filename) => {
    ev.preventDefault();
    onChange([filename, ...photo.filter((photo) => photo !== filename)]);
  };
  ...
  return (
    <>
      <div className="flex gap-2">
        <input
          type="text"
          placeholder={"Agregar usando un enlace ... jpg"}
          value={photoLink}
